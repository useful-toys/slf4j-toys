...
- **On `start()`** (gated by `MeterConfig.detectLeaks` and skipping the `UNKNOWN` category), `register(this)` creates a `MeterReference` — a `PhantomReference<Meter>` that snapshots the `Meter`'s identifying components (`category`, `operation`, `position`) and the message `Logger` — and adds it to a **static anchor set**. The `fullID` `String` itself is assembled from those components only inside `reportLeak()`, when a leak is actually confirmed; no `String` concatenation runs on the `start()` hot path.
...
- [TDR-0037: Single-Thread Meter Lifecycle Contract](TDR-0037-single-thread-meter-lifecycle-contract.md)
