...
- **Per-registration snapshot cost**: `register()` calls `Meter.getFullID()` on every `start()` even when no leak ever occurs. `getFullID()` now builds its identifier with plain string concatenation (lowered to `invokedynamic` `StringConcatFactory`) rather than `String.format(...)`, so the cost is much smaller than in the original design; the value is still computed eagerly and discarded on `deregister()`, and could be deferred into `reportLeak()` if it ever proves significant.
