        ...
        try {
            ...
            if (iteration > lastProgressIteration && ((now = System.nanoTime()) - lastProgressTime) > meterProgressPeriodNanoseconds) {
                ...
                if (julLogger.isLoggable(Level.INFO)) {
                    collectRuntimeStatus();
                    collectPlatformStatus();
                    julLogger.log(progressInfoStausLogRecord(Markers.MSG_PROGRESS, readableString(new StringBuilder()).toString()));
                    if (julLogger.isLoggable(Level.FINEST)) {
                        ...
                        } else if (julLogger.isLoggable(Level.FINEST)) {
...
//            this.threadStopName = currentThread.getName();
            if (julLogger.isLoggable(Level.WARNING)) {
                ...
                if (warnSlowness) {
                    julLogger.log(okWarnLogRecord(Markers.MSG_SLOW_OK, readableString(new StringBuilder()).toString()));
                } else if (julLogger.isLoggable(Level.INFO)) {
                    julLogger.log(okInfoStausLogRecord(Markers.MSG_OK, readableString(new StringBuilder()).toString()));
                }
                if (julLogger.isLoggable(Level.FINEST)) {
                    if (warnSlowness) {
                        julLogger.log(okTraceStatusLogRecord(Markers.DATA_SLOW_OK, write(new StringBuilder(), 'M').toString()));
                    } else {
                        julLogger.log(okTraceStatusLogRecord(Markers.DATA_OK, write(new StringBuilder(), 'M').toString()));
                    }
                }
                if (context != null) {
                    context.clear();
...
//            this.threadStopName = currentThread.getName();
            if (julLogger.isLoggable(Level.INFO)) {
                ...
                collectPlatformStatus();
                if (julLogger.isLoggable(Level.INFO)) {
                    julLogger.log(rejectInfoStausLogRecord(Markers.MSG_REJECT, readableString(new StringBuilder()).toString()));
                }
                if (julLogger.isLoggable(Level.FINEST)) {
                    julLogger.log(rejectTraceStatusLogRecord(Markers.DATA_REJECT, write(new StringBuilder(), 'M').toString()));
                }
                if (context != null) {
                    context.clear();
                }
...
//            this.threadStopName = currentThread.getName();
            if (julLogger.isLoggable(Level.SEVERE)) {
                collectRuntimeStatus();
                collectPlatformStatus();
                julLogger.log(failErrorStatusLogRecord(Markers.MSG_FAIL, readableString(new StringBuilder()).toString(), cause));
                if (julLogger.isLoggable(Level.FINEST)) {
                    julLogger.log(failTraceStatusLogRecord(Markers.DATA_FAIL, write(new StringBuilder(), 'M').toString()));
                }
                if (context != null) {
                    context.clear();
                }
    ...
    private static LogRecord failErrorStatusLogRecord(Marker marker, String message, Throwable cause) {
        ...
        logRecord.setParameters(new Object[]{marker.getName()});
