 ...
 *
 * <p>The servlet creates its own {@link Watcher} instance during initialization. By default the
 * watcher uses {@link WatcherConfig#name}. An alternative name may be configured through the
 * {@code slf4jtoys.watcher.name} servlet {@code <init-param>} in {@code web.xml}, which takes
 * precedence over {@link WatcherConfig#name} at initialization time. Other watcher settings (logger
 ...
 *     performance degradation or service unavailability. Concurrent requests that arrive while a
 *     collection is already in progress are rejected with HTTP 429 instead of being queued, which
 *     limits servlet-container thread contention; see {@link #doGet} and {@link #runWatcher()}.</li>
 * </ul>
...
public class WatcherServlet extends HttpServlet {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(WatcherServlet.class);
    ...
    protected void doGet(final HttpServletRequest request, final HttpServletResponse response) {
        try {
            ...
            if (collected) {
                ...
                LOGGER.info("WatcherServlet accessed while a collection is already in progress. Skipped.");
                response.setContentType("text/plain");
