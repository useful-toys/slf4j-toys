...
class MeterLifeCycleTest {
    ...
    class InvalidTransitions {
        @Test
        @DisplayName("should log inconsistency when stopping without starting")
        void shouldLogInconsistencyWhenStoppingWithoutStarting() {
            // Given: meter in Created state (not started)
            final Meter meter = new Meter(logger);
            assertMeterState(meter, false, false, null, null, null, null, 0, 0);

            // When: ok() is called
            meter.ok();
            // Then: state remains Created, and error is logged
            assertMeterState(meter, false, false, null, null, null, null, 0, 0);
            ...
            meter.fail("error");
            assertMeterState(meter, false, false, null, null, null, null, 0, 0);
            AssertLogger.assertEvent(logger, 2, Level.ERROR, Markers.INCONSISTENT_FAIL);
        }

        @Test
        @DisplayName("should log inconsistency when reporting progress without starting")
        void shouldLogInconsistencyWhenReportingProgressWithoutStarting() {
            // Given: meter in Created state
            final Meter meter = new Meter(logger);
        ...
        @Test
        @DisplayName("should log inconsistency when starting twice")
        void shouldLogInconsistencyWhenStartingTwice() {
            // Given: meter is Started
            final Meter meter = new Meter(logger);
            meter.start();
        ...
        @Test
        @DisplayName("should log inconsistency when performing actions after stop")
        void shouldLogInconsistencyWhenPerformingActionsAfterStop() {
            // Given: meter is Stopped (OK)
            final Meter meter = new Meter(logger);
            meter.start();
            meter.ok();
            assertMeterState(meter, true, true, null, null, null, null, 0, 0);
            ...
            meter.ok();
            AssertLogger.assertEvent(logger, 5, Level.ERROR, Markers.INCONSISTENT_OK);

            // When: reject() is called
            meter.reject("reason");
            AssertLogger.assertEvent(logger, 6, Level.ERROR, Markers.INCONSISTENT_REJECT);

            // When: fail() is called
            meter.fail("error");
            AssertLogger.assertEvent(logger, 7, Level.ERROR, Markers.INCONSISTENT_FAIL);

            // When: progress() is called
            meter.progress();
            AssertLogger.assertEvent(logger, 8, Level.ERROR, Markers.INCONSISTENT_PROGRESS);

            // Then: state remains Stopped (OK)
            assertMeterState(meter, true, true, null, null, null, null, 0, 0);
        }
    }
}
