...
- **Header** — `<type>(<scope>): <summary>`, summary in imperative mood ("implement", not "implemented"), ≤50 characters. See Types and Scopes below.
...
- **Attribution** — required on every AI-generated commit (see below).
...
Name the actual assistant and model that generated the commit — not a fixed placeholder tool name:

```
Co-Authored-By: <Assistant Name> <Model> <contact address>
```

For Claude Code, that's the harness default: `Co-Authored-By: Claude Sonnet 5 <noreply@anthropic.com>`. If the commit is generated by GitHub Copilot specifically, follow `.github/copilot-instructions.md`'s override instead (`Co-authored-by: GitHub Copilot using <model name>`). Apply the same convention consistently to any PR description opened for this commit (see `git-pull-request`).
...
- Header is imperative mood, ≤50 characters, correct `type(scope)`.
- Body explains what and why, not just what (the diff already shows what).
- Test coverage line states numbers actually observed.
- Attribution trailer is present and names the real assistant/model.
- The commit covers one concern — split unrelated changes into separate commits.
